...
const Historial = {
    ...
    async getAll({ tipo_equipo, tipo_accion, limit = 50, offset = 0 } = {}) {
        ...
        `;
        const params = [];
        if (tipo_equipo) { sql += ' AND h.tipo_equipo = ?'; params.push(tipo_equipo); }
        if (tipo_accion) { sql += ' AND h.tipo_accion = ?'; params.push(tipo_accion); }
        // FIX TiDB: No acepta ? como placeholder para LIMIT/OFFSET en queries preparadas.
        // Se embeben directamente con parseInt() para prevenir SQL injection.
        const safeLimit  = Math.max(1, parseInt(limit)  || 50);
        const safeOffset = Math.max(0, parseInt(offset) || 0);
        sql += ` ORDER BY h.fecha_accion DESC LIMIT ${safeLimit} OFFSET ${safeOffset}`;
    ...
    async countAll({ tipo_equipo, tipo_accion } = {}) {
        ...
        const params = [];
        if (tipo_equipo) { sql += ' AND tipo_equipo = ?'; params.push(tipo_equipo); }
        if (tipo_accion) { sql += ' AND tipo_accion = ?'; params.push(tipo_accion); }
        const rows = await query(sql, params);
        return rows[0].total;
    },

    async getByEquipo(tipo_equipo, id_equipo) {
        return query(`
            SELECT h.*,
                p.nombre AS nombre_persona,
                u.Nombre_ubicacion,
            ...
            ORDER BY h.fecha_accion DESC`, [tipo_equipo, id_equipo]);
    },

    async registrar({ tipo_equipo, id_equipo, tipo_accion, id_persona, id_ubicacion, valor_anterior, valor_nuevo, razon, id_usuario }) {
        if (!id_usuario) throw new Error('id_usuario es obligatorio para registrar en historial');
        return query(
    ...
    async actualizarUltimoMantenimiento(tipo_equipo, id_equipo) {
        const info = TABLA_MAP[tipo_equipo];
        if (!info) return;
        return query(
            `UPDATE ${info.tabla} SET ultimo_mantenimiento = CURDATE() WHERE ${info.pk} = ?`,
            [id_equipo]
        );
    },
