...
const Computador = {
    ...
    async existeCodigo(codigo, excluirId = null) {
        let sql = 'SELECT id_computador FROM Computadores WHERE codigo_activo = ?';
        const params = [codigo];
    ...
    async getStats() {
        const [porEstado, porMarca, porTipo] = await Promise.all([
            query("SELECT estado, COUNT(*) as total FROM Computadores WHERE activo = 'ACTIVO' GROUP BY estado"),
            query("SELECT marca_equipo as marca, COUNT(*) as total FROM Computadores WHERE activo = 'ACTIVO' GROUP BY marca_equipo ORDER BY total DESC LIMIT 10"),
            query("SELECT tipo_computador, COUNT(*) as total FROM Computadores WHERE activo = 'ACTIVO' GROUP BY tipo_computador")
        ]);
        return { por_estado: porEstado, por_marca: porMarca, por_tipo: porTipo };
    },
    ...
    async create(datos) {
        const result = await query(`
            ...
            [
                ...
                datos.modelo_equipo,
                datos.serial_equipo,
                datos.mac_wifi || 'No definido',
                datos.mac_ethernet || 'No definido',
                datos.direccion_ip,
                datos.tipo_computador || 'No definido',
                datos.procesador,
                datos.capacidad_ram,
                datos.tipo_modulo_ram || 'DDR4',
                datos.capacidad_disco,
                datos.tipo_disco || 'No definido',
                datos.sistema_operativo,
                datos.serial_teclado || 'GENERICO',
                datos.serial_mouse || 'GENERICO',
                datos.marca_monitor,
                datos.serial_monitor,
                datos.segunda_pantalla || false,
                datos.serial_2_monitor || 'N/A',
                datos.cargador_regulador || 'N/A',
                datos.estado || 'BUENO',
                datos.fecha_adquisicion,
                datos.fecha_garantia,
                datos.notas
            ]
        );
    ...
    async update(id, campos) { /* igual que tu versión original */ 
        // (mantengo tu lógica de allowed fields)
        const allowed = ['nombre_equipo','marca_equipo','modelo_equipo','serial_equipo','mac_wifi','mac_ethernet','direccion_ip','tipo_computador','procesador','capacidad_ram','tipo_modulo_ram','capacidad_disco','tipo_disco','sistema_operativo','serial_teclado','serial_mouse','marca_monitor','serial_monitor','segunda_pantalla','serial_2_monitor','cargador_regulador','estado','fecha_adquisicion','fecha_garantia','ultimo_mantenimiento','notas'];
        const fields = [], values = [];
    ...
    async asignar(id_computador, id_persona, id_ubicacion) {
        ...
        return query('INSERT INTO Computadores_persona (id_computador, id_persona, id_ubicacion, activo) VALUES (?,?,?,?)', 
            [id_computador, id_persona, id_ubicacion, 'ACTIVO']);
    },

    async desasignar(id_computador) {
        return query("UPDATE Computadores_persona SET activo = 'DADO DE BAJA', fecha_devolucion = CURDATE() WHERE id_computador = ? AND activo = 'ACTIVO'", [id_computador]);
    },
