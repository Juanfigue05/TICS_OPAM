...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al obtener ubicación' });
    }
});
...
router.post('/', authenticateToken, requireRole(['technician', 'admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al crear ubicación: ' + e.message });
    }
});
...
router.put('/:id', authenticateToken, requireRole(['technician', 'admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al actualizar ubicación: ' + e.message });
    }
});
...
router.delete('/:id', authenticateToken, requireRole(['admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al desactivar ubicación: ' + e.message });
    }
});
