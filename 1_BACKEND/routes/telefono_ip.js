...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al obtener teléfonos IP' });
    }
});

router.get('/stats', authenticateToken, async (req, res) => {
    try {
        const data = await Telefono_ip.getStats();
...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al obtener teléfono IP' });
    }
});
    ...
    try {
        ...
        if (id_persona && id_ubicacion) {
            ...
            await Historial.registrar({
                tipo_equipo: 'TELEFONO_IP',
                id_equipo: id_telefono_ip,
        ...
        res.status(201).json({
            success: true,
            message: 'Teléfono IP creado',
            data: { id_telefono_ip }
        });
    } catch (e) {
        res.status(500).json({ success: false, error: 'Error al crear teléfono IP: ' + e.message });
    }
});
...
router.put('/:id', authenticateToken, requireRole(['technician', 'admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al actualizar teléfono IP: ' + e.message });
    }
});
...
router.delete('/:id', authenticateToken, requireRole(['admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al dar de baja teléfono IP: ' + e.message });
    }
});
